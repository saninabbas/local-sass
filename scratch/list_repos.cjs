...
async function listUserRepos() {
  ...
  const reposRes = await request('GET', '/api/github/repositories', null, authHeaders);
  console.log("GitHub Repositories accessible with saved token:", JSON.stringify(reposRes.data, null, 2));
}

listUserRepos().catch(console.error);
